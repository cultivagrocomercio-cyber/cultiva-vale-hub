...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <div className="max-w-md text-center">
        <h1 className="text-xl font-semibold tracking-tight">Essa página não carregou</h1>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          ...
          <a
            href="/"
            className="inline-flex items-center justify-center rounded-full border bg-background px-5 py-2 text-sm font-semibold hover:bg-muted"
          >
            Início
          </a>
        </div>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Cultiva Vale Marketplace" },
      { name: "description", content: "O Ceasa digital do Vale do Ribeira: plantas, insumos e máquinas agrícolas direto do produtor." },
      { name: "author", content: "Cultiva Vale" },
      { property: "og:type", content: "website" },
      { property: "og:site_name", content: "Cultiva Vale Marketplace" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "theme-color", content: "#3f7a4f" },
    ],
    links: [
